...
source(here::here("code/functions/functions_compute_signal_14may.R"))
```
...
saveRDS(df_all_procrustes, here::here("output/neutral/test/df_all_procrustes.rds"))
#df_all_procrustes <- readRDS(here::here("output/selection/selection_df_all_procrustes.rds"))
...
#saveRDS(df_mantel, here::here("output/selection/neutral_df_mantel_1-61.rds"))
...
```{r}
decomposed_files <- list.files("decomposed_matrices/selection", full.names = TRUE, pattern = "sdv_matrices_roles_\\d+\\.rds")
...
decomposed_files <- decomposed_files[41:100]

all_cluster_results <- map_dfr(seq_along(decomposed_files), function(i) {
  cat("Analyzing simulation", i, "\n")
  sdv <- readRDS(decomposed_files[i])
  res_sim <- readRDS(list.files("simulations/selection", full.names = TRUE, pattern = "res_sim_\\d+\\.rds")[i])
...
saveRDS(all_cluster_results, here::here("output/selection/selection_all_cluster_results_41-100.rds"))
...
summary_cluster <- all_cluster_results %>%
  ...
  filter(!is.nan(mean_ARI) | !is.nan(mean_NMI))  # remove completely missing rows
