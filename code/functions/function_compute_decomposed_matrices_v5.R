...
generate_species_letters <- function(n) {
  # Generate letter names like A, B, ..., Z, AA, AB, ..., AZ, BA, ...
  letters <- toupper(letters)
  if (n <= 26) return(letters[1:n])
  combos <- expand.grid(letters, letters)
  all_names <- c(letters, paste0(combos$Var1, combos$Var2))
...
compute_result_matrices <- function(results_simulation, int, Smax, nbasals) {
  ...
  #### convert spp names to letters in presence matrix
  colnames(presence_matrix) <- generate_species_letters(Smax)
