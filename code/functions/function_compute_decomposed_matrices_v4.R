...
compute_result_matrices <- function(results_simulation, int, Smax, nbasals) {
  ...
  # Identify which elements are NOT NULL
  valid_indices <- which(!sapply(network_list, is.null))
  
  valid_indices <- which(!sapply(network_list, is.null))
  network_list <- network_list[valid_indices]
  list_anc_dist <- list_anc_dist[valid_indices]
  
  # 2. Convert names AFTER filtering
...
all_anc_dist <- results_simulation$list_anc_dist
  ...
  # Define helper function to convert integer to letter code
  int_to_letters <- function(n) {
    if (n <= 26) {
      return(LETTERS[n])
    } else {
  ...
  # Build full mapping: species number → letter name (as used in presence_matrix colnames)
  species_map <- setNames(sapply(1:Smax, int_to_letters), as.character(1:Smax))
  
  # Initialize the summary table
  species_life_summary <- data.frame(
    spp_number = all_species,
    spp_letter = species_map[as.character(all_species)],
    timestep_birth = NA,
    timestep_extinct = NA,
    n_descendants = 0,
  ...
  # Use the original full presence matrix before trimming
  full_presence_matrix <- results_simulation$presence_matrix
  
  # Convert column names to letters
  colnames(full_presence_matrix) <- sapply(1:Smax, function(x) {
    if (x <= 26) LETTERS[x] else paste0(LETTERS[(x - 1) %/% 26], LETTERS[(x - 1) %% 26 + 1])
  })
  
  # Fill birth and extinction using the full matrix
  for (i in 1:nrow(species_life_summary)) {
    spp_letter <- species_life_summary$spp_letter[i]
    
    if (!is.na(spp_letter) && spp_letter %in% colnames(full_presence_matrix)) {
      presence_vector <- full_presence_matrix[, spp_letter, drop = TRUE]
