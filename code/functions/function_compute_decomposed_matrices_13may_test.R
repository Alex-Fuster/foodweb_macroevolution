...
compute_decomposed_matrices_roles <- function(results_simulation, int, Smax, nbasals) {
  ...
  for (i in seq_along(list_anc_dist_all)) {
    ...
    network_alive <- if (i <= length(network_crop_list)) network_crop_list[[i]] else NULL
    if (is.null(network_alive) || nrow(network_alive) < 2 || ncol(network_alive) < 2) next
    ...
    svd_result <- tryCatch(svd(network_alive_cropped), error = function(e) NULL)
    
    if (is.null(svd_result)) next  # Skip on SVD failure
    ...
    # Defensive check: ensure U and V have compatible dimensions
    if (is.null(U) || is.null(V) || nrow(U) == 0 || ncol(U) < k || ncol(V) < k) {
      next  # skip this timestep if dimensions are incompatible
    }
    
    U_k <- U[, 1:k, drop = FALSE]
    V_k <- V[, 1:k, drop = FALSE]
    Ssqrt <- diag(sqrt(d[1:k]))
    
    # More defensive: ensure Ssqrt is 2D
    if (k == 1) {
      Ssqrt <- matrix(sqrt(d[1]), nrow = 1, ncol = 1)
    }
