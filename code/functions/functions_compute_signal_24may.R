...
compute_clustering_metrics_roles <- function(sdv_matrices, results_simulation, Smax = 1000, nbasals = 5) {
  ...
  for (i in seq_along(valid_timesteps))  {  # <-- only loop over existing matrices
    ...
    # Re-index cluster IDs to be safe for igraph
    phylo_vec <- match(phylo_clusters_nonbasal[valid_idx], unique(phylo_clusters_nonbasal[valid_idx]))
    interact_vec <- match(interaction_clusters_nonbasal[valid_idx], unique(interaction_clusters_nonbasal[valid_idx]))
