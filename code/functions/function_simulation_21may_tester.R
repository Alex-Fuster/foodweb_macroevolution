...
sim_model <- function(seed, pars, nsteps) {
  with(pars, {
    ...
    S <- 1
    ext_prob_log <- list()  # Store extinction probabilities for logging
    ...
    for (step in 2:nsteps) {
      ...
      if (length(present_non_basal) > 1) {
        ...
        if(SN < 1){
          ...
          # --- selection-based extinction calculation as before ---
          ...
          out_I_cropped <- rowSums(L_cropped)
          ...
          herbivores <- (colSums(L[1:Sbasal, present_non_basal, drop=FALSE] > 0) > 0) &
            (colSums(L[(Sbasal + 1):nrow(L), present_non_basal, drop=FALSE] > 0) == 0)
          predators <- (colSums(L[1:Sbasal, present_non_basal, drop=FALSE] == 0) &
                          (colSums(L[(Sbasal + 1):nrow(L), present_non_basal, drop=FALSE] > 0) > 0))
          omnivores <- (colSums(L[1:Sbasal, present_non_basal, drop=FALSE] > 0) > 0) &
            (colSums(L[(Sbasal + 1):nrow(L), present_non_basal, drop=FALSE] > 0) > 0)
          ...
          ext_prob_omn <- beta_ext * ext_prob_topdown[omnivores] +
            (1 - beta_ext) * transformed_similarity[omnivores]
          ...
          ext_prob_sel_full[present_non_basal] <- SN * ext_prob_neutral + (1 - SN) * ext_prob_sel_full[present_non_basal]
          
          
        }else if(identical(SN, 1)){
          
          ext_prob_sel_full <- numeric(Smax)
          ext_prob_sel_full[present_non_basal] <-  ext_prob_neutral
        }
        
       

        
        # --- Logging: Save ext prob distribution every 20 steps ---
        if (step %% 20 == 0) {
          ext_prob_log[[length(ext_prob_log) + 1]] <- ext_prob_sel_full[present_non_basal]
        }
