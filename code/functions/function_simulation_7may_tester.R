  ...
  with(pars, {
    ...
    for (step in 2:nsteps) {
      ...
      for (i in 1:Smax) {
        ...
        if (pres[step - 1, i] == 1 && dist_anc$A_E[i] != "E") {
          ...
          if (test_number < speciation_prob) {
            ...
            if (step %% 10 == 0) {
              cat(sprintf("Step %d | SN = %.1f | sum_I = %.4f | estab_prob_sel = %.4f | estab_prob_neutral = %.4f | used = %.4f (mut1)\n",
                          step, SN, sum_I,
                          (u_0pos + u_1pos * exp(-a_upos * sum_I)),
                          estab_prob_neutral,
                          estab_prob))
            }
      ...
      if (length(present_non_basal) > 1) {
        ...
        # --- define neutral extinction shortcut ---
        ext_prob_neutral <- 0.005
        random_number <- runif(length(present_non_basal))
        
        if (SN == 1) {
          # --- NEUTRAL: skip all interaction-based logic ---
          pres[step, present_non_basal] <- ifelse(random_number < ext_prob_neutral, 0, 1)
          
          if (step %% 10 == 0) {
            cat(sprintf("Step %d | S = %d | SN = %.1f | Neutral extinction only: prob = %.4f\n",
                        step, length(present_non_basal), SN, ext_prob_neutral))
          }
        ...
        if (step %% 10 == 0) {  # log every 10 steps to reduce output
          mean_sel <- mean(ext_prob_sel_full[present_non_basal] / multiplier, na.rm = TRUE)
          mean_combined <- mean(ext_prob_sel_full[present_non_basal], na.rm = TRUE)
          mean_neutral <- 0.005  # your fixed neutral value
          
          cat(sprintf(
            "Step %d | S = %d | SN = %.1f | Mean ext prob (sel): %.4f | Mean ext prob (neutral): %.4f | Combined: %.4f\n",
            step, length(present_non_basal), SN, mean_sel, mean_neutral, mean_combined
          ))
        }
