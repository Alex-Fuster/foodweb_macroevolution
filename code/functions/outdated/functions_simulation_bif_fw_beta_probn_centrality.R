...
sim_model = function(seed, pars, nsteps) {
  ...
  with(pars, {
    ...
    eig_centrality_matrix <- matrix(NA, nrow = nsteps, ncol = Smax)
    btw_centrality_matrix <- matrix(NA, nrow = nsteps, ncol = Smax)
...
ext_prob_sel <- beta_ext * ext_prob_topdown + (1 - beta_ext) * transformed_similarity
      ...
      present_indices <- which(pres[step, ] == 1)
      
      if (length(present_indices) > 1) {
        L_current <- get_L_mat(basal, pars, traits_mat)[c(1:Sbasal, Sbasal + present_indices), present_indices]
        adj_matrix <- L_current[(Sbasal + 1):nrow(L_current), ] != 0
        
        g <- igraph::graph_from_adjacency_matrix(adj_matrix, mode = "directed", diag = FALSE)
        
        eig_cent <- igraph::eigen_centrality(g, directed = TRUE)$vector
        btw_cent <- igraph::betweenness(g, directed = TRUE)
        
        eig_centrality_matrix[step, present_indices] <- eig_cent
        btw_centrality_matrix[step, present_indices] <- btw_cent
      }
    ...
    # Record death time
    df_traits_summary$death_step <- sapply(df_traits_summary$species_id, function(sid) {
      extinct_step <- which(pres[, sid] == 0 & dplyr::lag(pres[, sid], default = 1) == 1)
      if(length(extinct_step) > 0) return(min(extinct_step)) else return(nsteps)
    })
    
    # Compute longevity
    df_traits_summary$longevity <- df_traits_summary$death_step - df_traits_summary$birth_step
    
    # Count number of direct descendants
    desc_counts <- table(anc[, 2])
    df_traits_summary$descendants <- desc_counts[as.character(df_traits_summary$species_id)]
    df_traits_summary$descendants[is.na(df_traits_summary$descendants)] <- 0
    
    # Compute diversification rates
    df_traits_summary$speciation_rate <- df_traits_summary$descendants / df_traits_summary$longevity
    df_traits_summary$extinction_rate <- 1 / df_traits_summary$longevity
    
    
    ##########################
    
    # Extract once to avoid repeated indexing
    birth_steps <- df_traits_summary$birth_step
    death_steps <- df_traits_summary$death_step
    species_ids <- df_traits_summary$species_id
    
    # Eigenvector centrality
    df_traits_summary$eigen_centrality <- mapply(function(sid, b, d) {
      mean(eig_centrality_matrix[b:d, sid], na.rm = TRUE)
    }, sid = species_ids, b = birth_steps, d = death_steps)
    
    # Betweenness centrality
    df_traits_summary$betweenness_centrality <- mapply(function(sid, b, d) {
      mean(btw_centrality_matrix[b:d, sid], na.rm = TRUE)
    }, sid = species_ids, b = birth_steps, d = death_steps)
    ...
    list(pres = pres, 
         ...
         df_guild = df_guild,
         df_traits_summary = df_traits_summary)
