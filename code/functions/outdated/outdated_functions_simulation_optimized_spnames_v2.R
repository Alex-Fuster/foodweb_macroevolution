...
sim_model <- function(seed, pars, nsteps) {
  with(pars, {
    ...
    # Species still alive at final timestep
    alive_in_pres <- colnames(pres)[pres[final_step, ] == 1]
    
    # Species marked alive ("A") in dist_anc
    alive_in_dist_anc <- dist_anc$spp[dist_anc$A_E == "A"]
    
    # Check that the two sets match
    if (!setequal(alive_in_pres, alive_in_dist_anc)) {
      warning(paste0("FINAL CHECK: Alive species mismatch between presence matrix and dist_anc. ",
                     "Species in pres but not in dist_anc: ", 
                     paste(setdiff(alive_in_pres, alive_in_dist_anc), collapse = ", "),
                     " --- Species in dist_anc but not in pres: ",
                     paste(setdiff(alive_in_dist_anc, alive_in_pres), collapse = ", ")))
    }
