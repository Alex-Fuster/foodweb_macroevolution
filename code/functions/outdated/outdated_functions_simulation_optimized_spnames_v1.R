...
sim_model = function(seed, pars, nsteps) {
  with(pars, {
    ...
    for(step in 2:nsteps) {
      ...
      extinct_in_dist_anc <- which(dist_anc$A_E == "E")
      
      if (!all(extinct_in_pres %in% extinct_in_dist_anc)) {
        warning(paste0("Mismatch detected at step ", step, ": ",
                       "Species extinct in pres but not marked 'E' in dist_anc: ",
                       paste(setdiff(extinct_in_pres, extinct_in_dist_anc), collapse = ", ")))
      }
      
      if (!all(extinct_in_dist_anc %in% extinct_in_pres)) {
        warning(paste0("Mismatch detected at step ", step, ": ",
                       "Species marked 'E' in dist_anc but not extinct in pres: ",
                       paste(setdiff(extinct_in_dist_anc, extinct_in_pres), collapse = ", ")))
      }
