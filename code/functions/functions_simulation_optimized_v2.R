...
ext_prob_sel <- beta_ext * ext_prob_topdown + (1 - beta_ext) * transformed_similarity
      ...
      #  print(paste("spp with no resources:", spp_noresources))
       # print(paste("p(ext) for spp with no resources:", ext_prob_sel[spp_noresources]))
        ...
        # Update dist_anc for species that went extinct in this timestep
        extinct_now <- which(pres[step - 1, ] == 1 & pres[step, ] == 0)
        if (length(extinct_now) > 0) {
          dist_anc[extinct_now, "A/E"] <- "E"
        }
        
        
        # Sanity check: presence vs ancestry status
        alive_in_pres <- which(pres[step, ] == 1)
        alive_in_anc  <- which(dist_anc$`A/E` == "A")
        
        if (!setequal(alive_in_pres, alive_in_anc)) {
          warning(paste("Mismatch detected at step", step))
       #   print(setdiff(alive_in_pres, alive_in_anc)) # pres alive but anc says dead
       #   print(setdiff(alive_in_anc, alive_in_pres)) # anc alive but pres says dead
        }
      ...
      # ---- Centralized ancestry status update ----
      
      # Ensure dist_anc is fully aligned with presence at current step
      # Overwrite everything based on presence matrix
      dist_anc$`A/E` <- "E"
      dist_anc[which(pres[step, ] == 1), "A/E"] <- "A"
      
      # === Optional: check mismatch for debugging ===
      alive_in_pres <- which(pres[step, ] == 1)
      alive_in_anc  <- which(dist_anc$`A/E` == "A")
      
      if (!setequal(alive_in_pres, alive_in_anc)) {
        warning(paste("Mismatch detected at step", step))
        cat("[In pres but not in dist_anc]:", setdiff(alive_in_pres, alive_in_anc), "\n")
        cat("[In dist_anc but not in pres]:", setdiff(alive_in_anc, alive_in_pres), "\n")
      }
      ...
      # 1. Stop if 0 species at timestep 10
      if (step == 10 && sum(pres[step, ]) == 0) {
        cat("Stopped: 0 species at timestep 10\n")
        return(NULL)
      }
      
      # 2. Stop if 0 species at any timestep >10
      if (step > 10 && sum(pres[step, ]) == 0) {
        cat("Stopped: 0 species after timestep 10\n")
        return(NULL)
      }
      
      # 3. Mark bad simulation if <10 species at timestep 50
      if (step == 50 && sum(pres[step, ]) < 10) {
        cat("Stopped: Less than 10 species at timestep 50\n")
        return(NULL)
      }
      
      # 4. Mark bad simulation if <30 species at final timestep
      ...
      # 4. Mark bad simulation if <30 species at final timestep
      if(step == nsteps && sum(pres[step,]) < 40){
        cat("Simulation not valid due to low species presence.\n")
        return(NULL)  # ❗ This is crucial
      }
