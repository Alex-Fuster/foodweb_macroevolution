...
compute_decomposed_matrices <- function(results_simulation, int, Smax, nbasals) {
  ...
  if (length(list_anc_dist) != length(network_list)) {
    cat("PROBLEM - list_anc_dist and network_list have different lengths\n")
  }
  if (nrow(presence_matrix) != length(network_list)) {
    cat("PROBLEM - presence_matrix rows and network_list length mismatch\n")
  }
  # ---------------
  
  if (int == "foodweb") {
    network_list <- lapply(network_list, eliminate_basals, nbasals = nbasals)
  }
  ...
  for (i in seq_along(list_anc_dist)) {
    ...
    if (length(missing_species) > 0) {
      ...
      # Remove missing species from alive_species list
      alive_species <- setdiff(alive_species, missing_species)
    }
    
    # Now crop both phylogeny and network to the final alive_species
    ...
    network_cropped <- network_list[[i]][alive_species, alive_species]
    list_net_present_spp.letters[[i]] <- network_cropped
  }
