...
compute_clustering_metrics_roles <- function(sdv_matrices, results_simulation, Smax = 1000, nbasals = 5) {
  ...
  for (i in seq_along(list_networks_sppnames_letters)) {  # <-- only loop over existing matrices
    ...
    optim_n <- optim_result$optim_n
    
    if (is.null(optim_n) || is.na(optim_n) || !is.numeric(optim_n) || optim_n < 2) {
    ...
    ARI[i] <- compare(interaction_clusters_nonbasal, phylo_clusters_nonbasal, method = "adjusted.rand")
    NMI[i] <- compare(interaction_clusters_nonbasal, phylo_clusters_nonbasal, method = "nmi")
    timestep[i] <- i
  }
