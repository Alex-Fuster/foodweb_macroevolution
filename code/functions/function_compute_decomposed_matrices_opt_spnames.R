...
compute_decomposed_matrices <- function(results_simulation, int, Smax, nbasals) {
  ...
  # No conversion of names needed anymore
  
  ## ------------------- Check
  if (length(list_anc_dist) != length(network_list)) {
    print("PROBLEM - list_anc_dist != list_networks")
  }
  ## -------------------
  
  # Discard the same timesteps in the presence matrix
  presence_matrix <- presence_matrix[(final.discarded_timestep+1):length(results_simulation$list_anc_dist), ]
  
  ## ------------------- Check
  if (nrow(presence_matrix) != length(network_list)) {
    print("PROBLEM - presence matrix, list phylo dist and list interaction networks don't have the same n_steps")
  }
  ## -------------------
  ...
  for (i in 1:length(list_anc_dist)) {
    cat("step", i, "\n")
    
    alive_species <- names(which(presence_matrix[i, ] == 1))
    
    # 🛠️ Build full phylogeny (all species, alive + dead)
    ...
    # 🛠️ Now try cropping to alive species
  ...
  for (i in 1:length(network_list)) {
    list_net_present_spp.letters[[i]] <- network_list[[i]][names(which(presence_matrix[i, ] == 1)), names(which(presence_matrix[i, ] == 1))]
  }
  ...
  if (length(list_svd_pred) != length(list_svd_eigen.phy)) {
    print("PROBLEM - length mismatch between interaction and phylo matrices")
  }
  ## -------------------
...
#     alive_species <- names(which(presence_matrix[i, ] == 1))
#     
#     # Keep only species that are present in phylo.corr
#     valid_species <- intersect(alive_species, rownames(phylo.corr))
#     
#     if (length(valid_species) < 3) {
#       warning(paste("Skipping step", i, "- too few valid species for phylo corr"))
#       next
#     }
#     
#     list_phylo.corr_cropped[[i]] <- phylo.corr[valid_species, valid_species]
#     
#     ###########
#     
#     
